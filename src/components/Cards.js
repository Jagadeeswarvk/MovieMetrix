...
const Cards = () => {
  ...
  useEffect(() => {
    async function getData() {
      ...
      _data.forEach((doc) => {
        return setData((prev) => [...prev, { ...(doc.data()),id:doc.id}]);
      })
      setLoading(false);
    }
    getData();
    ...
    <div className="flex flex-wrap justify-between px-3 mt-2">
      {
        loading ? <div className="w-full flex justify-center items-center h-96"><ThreeDots height={40} color="white" /></div> :
          data.map((e, i) => {
            return (
              <Link to={`/detail/${e.id}`}>
                <div className="card font-medium shadow-lg p-2 hover:-translate-y-2 cursor-pointer mt-6 transition-all duration-500">
